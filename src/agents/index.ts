...
export class WorkflowAgents {
   ...
   */
  public openai(...params: CustomModelParameters) {
    const [modelParams, providerSettings] = params;
    const { baseURL, apiKey, callSettings, ...otherSettings } = providerSettings ?? {};

    const openaiModel = this.AISDKModel({
      context: this.context,
      provider: createOpenAI,
      providerParams: { baseURL, apiKey, ...otherSettings },
      agentCallParams: callSettings,
    });

    return openaiModel(modelParams);
  }
